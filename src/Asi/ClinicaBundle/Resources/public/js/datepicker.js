...
jQuery(document).ready(function() {
  $('select[id$=year]').each(function(i) {
    ...
    $('<a/>').attr('id', 'datepick_' + this.id.slice(0, -4)).attr('href', '#').append($('<img/>').attr('src', '/img/calendar.png')).insertAfter(this).addDatePicker(this.id.slice(0, -4), $(this).children(':eq(1)').val(), $(this).children(':last').val());
  });
});
